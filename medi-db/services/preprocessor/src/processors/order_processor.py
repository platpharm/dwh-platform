...
class OrderProcessor:
    ...
    def _get_orders_detail_from_cdc(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        for doc in self.es.scroll_search(
            ...
            order_qty = doc.get("order_qty")
            order_price = doc.get("order_price")
            results.append({
                "id": doc.get("id"),
                "order_id": doc.get("order_id"),
                "product_id": doc.get("product_id"),
                "order_qty": order_qty if order_qty is not None else 0,
                "order_price": order_price if order_price is not None else 0,
    ...
    def _index_to_es(self, df: pd.DataFrame, batch_size: int = 5000) -> int:
        ...
        for i in range(0, len(documents), batch_size):
            ...
            try:
                ...
                if failed:
                    logger.warning(f"Batch {i//batch_size + 1}: {len(failed)} documents failed to index")
            except helpers.BulkIndexError as e:
                total_indexed += e.completed
                logger.warning(
                    f"Batch {i//batch_size + 1}: {len(e.errors)} documents failed to index"
                )
            except Exception as e:
