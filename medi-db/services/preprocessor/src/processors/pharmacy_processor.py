...
class PharmacyProcessor:
    ...
    def _get_accounts_from_cdc(self) -> List[Dict[str, Any]]:
        ...
        for doc in self.es.scroll_search(
            ...
            lat_raw = doc.get("lat") or doc.get("latitude")
            lng_raw = doc.get("lng") or doc.get("longitude")
    ...
    def _merge_order_statistics(
        ...
        if order_stats:
            stats_df = pd.DataFrame.from_dict(order_stats, orient="index")
            stats_df.index.name = "_pharmacy_id"
            stats_df = stats_df.reset_index()

            df["_pharmacy_id"] = df["id"].astype(str)
            df = df.merge(
                stats_df,
                on="_pharmacy_id",
                how="left",
            ...
            for col in ["total_orders", "total_amount", "avg_order_amount", "order_frequency", "unique_products"]:
                stats_col = f"{col}_stats"
                if stats_col in df.columns:
                    df[col] = df[stats_col].fillna(df[col])
                    df.drop(columns=[stats_col], inplace=True)

            df.drop(columns=["_pharmacy_id"], inplace=True)
            stats_count = stats_df["_pharmacy_id"].isin(df["id"].astype(str)).sum()
        else:
            stats_count = 0
