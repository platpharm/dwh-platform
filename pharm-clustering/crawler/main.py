...
# 프로젝트 루트 경로를 sys.path에 추가 (어디서 실행하든 임포트 가능)
...
def import_collector_class(collector_name: str) -> Optional[type]:
    ...
    # 시도할 임포트 경로 목록 (우선순위 순)
    import_paths = [
        f"crawler.{module_name}",  # 프로젝트 루트에서 실행 시
        module_name,               # crawler 디렉토리에서 실행 시
    ]
