...
async def preprocess_products():
    ...
    try:
        ...
        total_count = (
            product_result.get("processed_count", 0) +
            keyword_result.get("mapping_count", 0)
        )

        success = product_result["success"] and keyword_result["success"]

        return PreprocessResponse(
            success=success,
            processed_count=total_count,
            message=f"Product preprocessing: {product_result['message']}. "
                    f"Keyword extraction: {keyword_result['message']}"
        )
