...
def fetch_product_summary(product_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "target_count": aggs.get("target_count", {}).get("count", {}).get("value", 0),
...
def create_cluster_distribution_chart(df: pd.DataFrame) -> px.bar:
    """클러스터별 약국 분포"""
    cluster_col = None
    for col in ["cluster_id", "cluster", "pharmacy_cluster"]:
...
def render_page():
    ...
    if summary:
        ...
        with metric_cols[1]:
            st.metric("타겟 약국", f"{int(summary.get('target_count', 0)):,}")

        with metric_cols[2]:
            target_rate = (summary.get('target_count', 0) / summary.get('total_pharmacies', 1)) * 100 if summary.get('total_pharmacies', 0) > 0 else 0
            st.metric("타겟 비율", f"{target_rate:.1f}%")
    ...
    with chart_col2:
        ...
        if cluster_fig:
            ...
            if "is_target" in df.columns:
                ...
                target_counts["label"] = target_counts["is_target"].map({True: "타겟", False: "비타겟"})
    ...
    # 타겟 여부
    if "is_target" in df.columns:
        display_columns.append("is_target")
        column_config["is_target"] = st.column_config.CheckboxColumn("타겟", default=False)

    # 클러스터 정보
    for cluster_col in ["cluster_id", "cluster", "pharmacy_cluster"]:
    ...
    # 추가 정보 (있는 경우)
    optional_cols = ["region", "address", "pharmacy_id"]
    ...
    download_cols = st.columns([2, 2, 6])
    ...
    with download_cols[1]:
        ...
        if "is_target" in df.columns:
            ...
            if len(target_df) > 0:
                ...
                st.download_button(
                    label=f"타겟 약국만 다운로드 ({len(target_df)}건)",
                    data=csv_target,
                    file_name=f"target_pharmacies_{selected_product_id}.csv",
                    mime="text/csv",
                )
